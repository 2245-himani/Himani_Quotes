...
import com.example.himani_quotes.databinding.FragmentHomeBinding
...
class Home_Fragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        for (i in quotes.indices) {
            val qoutes = Qoutes_moden(quotes[i], authorName[i], "Author 1", false, id)
            quotesList.add(qoutes)
        }


        val quotesAdapter = QuotesListAdapter(requireContext(), quotesList)
        var layoutManager = LinearLayoutManager(requireContext())
        binding.risi1.layoutManager = layoutManager
        binding.risi1.adapter = quotesAdapter
